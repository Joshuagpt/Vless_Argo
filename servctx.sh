...
// - 能通过 exit 事件感知到崩溃并在日志里给出清晰提示，还可以有限次数自动重启
function startXray(xrayBinPath, configPath) {
  ...
  function spawnOnce() {
    ...
    child.on('exit', (code, signal) => {
      console.error(`xray 子进程退出(code=${code}, signal=${signal})`);
      if (restarts < MAX_RESTARTS) {
        restarts++;
        console.error(`xray 将在2秒后自动重启(第${restarts}/${MAX_RESTARTS}次)`);
        setTimeout(spawnOnce, 2000);
      } else {
        console.error('xray 自动重启次数已达上限，不再重启，请检查 config.json 或上面的 [xray] 日志排查原因');
      }
    });
...
async function startServer() {
  ...
  const xrayBaseUrl = arch === 'arm64'
    ? 'https://github.com/eooce/test/releases/download/freebsd-arm64'
    : 'https://github.com/eooce/test/releases/download/freebsd';
  const xrayBinPath = await downloadLibrary(`${xrayBaseUrl}/web`, 'web');
  try { fs.chmodSync(xrayBinPath, 0o755); } catch (e) { /* ignore */ }
